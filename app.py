...
def predict():
    if request.method ==  'POST':
        ...
        Mobile_no = str(request.form['Mobile No.'])
        ...
        if(prediction=="N"):
            prediction="Your loan is not approved"
            return render_template("prediction.html", prediction_text="{}".format(prediction))
        else:
            prediction="Congratulations!\nYour loan approved"
            return render_template("About.html",name=cname,gmail= Mail,Mob=Mobile_no,LoanAmount = LoanAmount*1000, Loan_Amount_Term=Loan_Amount_Term)
